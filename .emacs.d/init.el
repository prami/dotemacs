...
;nowa linia niezaleznie od poz kursora
;(defun new-shift-line()
;        (interactive)
;        (move-end-of-line 1)
;        (open-line 1)
;        (next-line 1)
;)
;(global-set-key (kbd "S-SPC") 'new-shift-line)
;przesun linie w dol
(defun line-down ()
   (interactive)
   (save-excursion
...
(defun line-up ()
   ...
   (yank)
   (previous-line))
(global-set-key (kbd "C-S-<up>") 'line-up)
;kopiowanie linii
(defun copy-line()
  (interactive)
  (move-beginning-of-line 1)
...
(add-to-list  'load-path "~/.emacs.d/plugins");moje
(load "zenburn") ;ja to dodalem
(zenburn) ;moje
;;fulscreen oddzielny plik F11
(load "fullscreen")
;;(require 'color-theme)
...
;; ecb tutaj linijki odkomentowac
...
(custom-set-faces
  ...
  ;; Your init file should contain only one such instance.
  ;; If there is more than one, they won't work right.
 '(default ((t (:inherit nil :stipple nil :background "#3f3f3f" :foreground "#dcdccc" :inverse-video nil :box nil :strike-through nil :overline nil :underline nil :slant normal :weight normal :height 120 :width normal :foundry "unknown" :family "Inconsolata")))))
